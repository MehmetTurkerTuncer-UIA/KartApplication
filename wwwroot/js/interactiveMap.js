...
function searchAddress() {
  ...
  if (address) {
    var url = `https://nominatim.openstreetmap.org/search?format=json&limit=1&q=${encodeURIComponent(
      address
    )}`;
...
document.getElementById("searchInput").addEventListener("input", function () {
  ...
  if (query.length > 0) {
    var url = `https://nominatim.openstreetmap.org/search?format=json&addressdetails=1&limit=5&q=${encodeURIComponent(
      query
    )}`;
          ...
          if (item.display_name.includes("Norveç")) {
            ...
            option.addEventListener("click", function () {
              document.getElementById(
                "searchInput"
              ).value = `${street} ${house_number}, ${city}, ${county}, ${country}`;
              suggestions.innerHTML = "";
